...
sub <- pokemon_numeric %>% select(., c(starts_with("against"), hp))
#pokemon_kmeans <- run_clust(pokemon_numeric, 4)
pokemon_kmeans <- run_clust(sub, 6)

# Expand pallette to make silhouette plot
#colourCount = length(unique(pokemon_data$type1))
...
# Plot clusters facetted on type
plot_data <- cbind(pokemon_kmeans$cluster, pokemon_data) %>%  
    mutate(cluster = as.character(pokemon_kmeans$cluster), pokemon_type = as.character(type1)) %>% 
...
fviz_nbclust(pokemon_numeric, kmeans, method = "silhouette", k.max = 32) + theme_classic()
```
